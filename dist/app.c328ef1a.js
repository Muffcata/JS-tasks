...
var wrapper = document.querySelector(".pokemon-wrapper");
...
var generatorSingleCard = function generatorSingleCard(singleCardData) {
...
var generateStrongestInfo = function generateStrongestInfo(data) {
  var resString = "";
  data.forEach(function (e, i) {
    return resString += i === data.length - 1 ? "and ".concat(e.name, " with ").concat(e.hp, " hit points") : "".concat(e.name, " with ").concat(e.hp, " hit points,");
  });
  footer.textContent = "Strongest Pokemons are: ".concat(resString);
};
...
function generatorCards(data) {
  ...
  //Filter Cards
  var strongPokemonsOnly = data.data.filter(function (el) {
    return (
  ...
  strongPokemonsOnly.sort(function (a, b) {
    return (
      //We take 2 cards and substract from each other
      parseInt(a.hp) - parseInt(b.hp)
    );
  });
  strongPokemonsOnly.forEach(function (el) {
    generatorSingleCard(el);
  });

  //REDUCE (example to exercise)
  var newDataObjects = strongPokemonsOnly.reduce(function (acc, val) {
